...
class Installment
{
    ...
    /**
     * Checks if this installment is the last one.
     *
     * @return boolean
     */
    ...
    /**
     * Calcs repayments and interestes for this installment.
     *
     * @return self
     */
    public function calc(): self
    {
        ...
        foreach ($ticks as $tick) {
            ...
            if ($schedule->getRepaymentsStyle() === 'annuit' && $schedule->getIsCalcDaily() === true) {
                ...
                // Get daily interests.
                $perc  = ( $period->getLength() * $tick['percentage'] );
                $rate  = ( ( $tick['annualRate'] / $period->getDaysInYear() ) * $perc );
